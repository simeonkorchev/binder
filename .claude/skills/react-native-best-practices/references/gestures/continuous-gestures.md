...
All gestures use `Animated` from `react-native-reanimated`. Gesture objects are plain values here — the React Compiler memoizes them; never wrap in `useMemo`.
...
// v2
const pan = Gesture.Pan().averageTouches(true).onUpdate(...);
```
...
// v2
const pinch = Gesture.Pinch().onUpdate(...).onEnd(...);
const pan = Gesture.Pan().onBegin(...).onUpdate(...);
...
// v2
const tap = Gesture.Tap().onEnd(() => { scheduleOnRN(setState, value); });
```

...
> Examples inline `style` for brevity. In client-mobile, static styles live in `StyleSheet.create` (`.claude/rules/005-mobile.md`); an animated style — a `useAnimatedStyle` result or a shared value — is the one thing passed inline.
...
// v2
const doubleTap = Gesture.Tap().numberOfTaps(2).onEnd(() => { scheduleOnRN(onDoubleTap); });
const singleTap = Gesture.Tap().numberOfTaps(1).onEnd(() => { scheduleOnRN(onSingleTap); });
...
// v2
const tap = Gesture.Tap().hitSlop(20).onEnd(() => { ... });
```

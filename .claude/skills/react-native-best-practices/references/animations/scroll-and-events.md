...
Automatically detects horizontal or vertical scroll. Works with `ScrollView`, `FlatList`, and `FlashList (not a dependency here — use `FlatList`)`. The ref can be changed at runtime.

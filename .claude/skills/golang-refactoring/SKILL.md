...
- **Execute mode** — one branch, one PR per atomic change, landed sequentially. In a Routine everything happens in this session and this checkout (`/go-quality-sweep`, Hard boundaries); interactively, sub-agents only when the user asks.
- **Simple-sweep mode** — a single mechanical, behavior-preserving transform applied tree-wide, proven by `make lint && make test`.
...
- **Load `.claude/rules/004-security.md` (and `golang-safety` for internal-correctness risk) whenever a step changes code logic, not just its shape.**
...
- → See `.claude/skills/golang-naming/SKILL.md` for what to rename identifiers _to_ — this skill owns _how_ to apply a rename safely at scale.
- → See `.claude/rules/001-architecture.md` for target directory/package layout — this skill owns the mechanics of moving code there without breaking callers.
- → See `.claude/skills/golang-modernize/SKILL.md` for version-driven idiom updates (`interface{}`→`any`, `slices`/`maps`) — a distinct concern from structural refactoring, though it shares the same tool-first discipline.
- → See `.claude/skills/golang-code-style/SKILL.md` for control-flow clarity and function-shape rules this skill helps you apply mechanically.
- → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for target patterns (options struct, DI, consumer-side interfaces) this skill helps you migrate toward.
- → See `.claude/rules/006-testing.md` and `002-go-conventions.md` §9 (Ginkgo/Gomega) for the test-writing practices that make the safety net in this skill trustworthy.
- → See `.golangci.yml` (the pinned linter set; `make lint`) for configuring `golangci-lint`, run here only as a post-step verification gate.
- → See `.claude/rules/004-security.md` (and `golang-safety`) for reviewing any step that changes code logic, not just its shape.

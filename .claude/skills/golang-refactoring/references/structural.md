...
func Process(s Store) error { /* uses s.Store, no import of package y */ }
```

```go
// package y — unchanged, already satisfies x.Store implicitly
type Store struct{ /* ... */ }
...
- **Caveat: this is a heuristic, not dogma.**
  ...
  - → See `.claude/rules/001-architecture.md` for the directory/package layout conventions this section assumes, and `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for judging when introducing an interface is the right call versus premature abstraction.
...
- The refactor is toward explicit construction: a constructor function that returns a struct, with dependencies passed in as parameters rather than reached for through a package-level variable or `init()`-populated singleton.
  ...
  - → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for constructor and dependency-injection patterns, and [safety-net.md](safety-net.md) in this skill for how this same seam is what a Feathers-style characterization test exploits to get coverage before a risky change.
...
- → See `.claude/rules/001-architecture.md` for directory/package layout conventions.
- → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for when an interface is the right design choice versus premature abstraction, and for constructor/DI patterns.

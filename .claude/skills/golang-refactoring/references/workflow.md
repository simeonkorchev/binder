...
- Workspace symbol search and `gopls codeaction` surface the mechanical options available at each site — its day-to-day mechanics (rename, browsing references, call hierarchy) are owned by `gopls` (optional tooling; the gate is `make lint && make test`).
...
- The same one-row-one-concern discipline holds even within a single letter:
  ...
  - a move is verified by gopls plus a green build/test run, while an optimization needs benchmarks and a closer read for subtle correctness changes
...
3. A human reviews and merges each of these small PRs into `refactor/<topic>` at their own pace.
   ...
   - For any PR that changes code logic rather than just its shape, load `.claude/rules/004-security.md` (and `golang-safety` for internal-correctness risk) alongside this skill before approving it, since a logic change can introduce a vulnerability or a bug that a purely mechanical refactor never could.
...
- → See `.claude/rules/004-security.md` (and `golang-safety`) for reviewing any PR that changes code logic, per Step 3 above.

...
- **MEDIUM coverage (~40-80%)** — harden the blast radius before refactoring it, not after.
  ...
  - Add targeted characterization specs (Ginkgo, external test package, counterfeiter fakes — `.claude/rules/006-testing.md`) covering precisely those paths, confirm they pass against the current code, and only then refactor.
...
→ See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for constructor and dependency-injection patterns this move builds on, and [catalog.md](catalog.md) in this skill for the Sprout/Wrap mechanics that typically pair with a freshly introduced seam.
...
go test -race ./...                          # concurrency changes — see .claude/rules/006-testing.md and 002-go-conventions.md §9 (Ginkgo/Gomega) for race-detector and testing/synctest mechanics
...
- → See `.claude/rules/006-testing.md` and `002-go-conventions.md` §9 (Ginkgo/Gomega) for general test-writing craft, race-detector mechanics, and `testing/synctest` — this file assumes them as a baseline and only covers when a refactor's safety net needs to reach for them.

...
- The full code-action reference, CLI invocation, safety behavior, and MCP server setup belong to `gopls` (optional tooling; the gate is `make lint && make test`) — this file covers the tools that skill doesn't.
...
- As of Go 1.26, `go fix` is rewritten onto the `go/analysis` framework and has converged with `go vet` — this is where the `modernize` fixer suite lives (`rangeint`, `mapsloop`, `minmax`, `any`, `stringscut`, `fmtappendf`, `omitzero`, and more; → See `.claude/skills/golang-modernize/SKILL.md` for the idiom-by-idiom breakdown).

...
→ See `.claude/skills/golang-naming/SKILL.md` for what to rename identifiers _to_. This entry covers only _how_ to apply the rename safely at scale.
...
→ See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for converting a long parameter list into an options struct rather than just reordering or trimming it.
...
→ See `.claude/rules/001-architecture.md` for target package layout. See [structural.md](structural.md) for the full type-alias gradual-repair recipe.
...
→ See `.claude/rules/001-architecture.md` for target package layout.
...
→ See `.claude/skills/golang-code-style/SKILL.md` for the full early-return style rule this refactor works toward.
...
→ See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for functional options as the alternative when the struct exists to configure construction rather than to group a plain data clump.
...
→ See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for the target interface-dispatch pattern.
...
- → See `.claude/skills/golang-naming/SKILL.md` for what to rename identifiers _to_.
- → See `.claude/rules/001-architecture.md` for target package/directory layout.
- → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for options structs, consumer-side interfaces, and interface-dispatch patterns referenced throughout this catalog.
- → See `.claude/skills/golang-code-style/SKILL.md` for the guard-clause/early-return style rule.

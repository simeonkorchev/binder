...
> Applies only to plain-`testing` `_internal_test.go` files (white-box helpers, with a `white-box:` reason). Specs are Ginkgo/Gomega — `Describe/When/It` sentences, one `Test<Pkg>` bootstrap per package (`.claude/rules/006-testing.md`, `002` §9).

...
| Interface | by role; `-er` only for a genuine agent | `Store`, `PermissionService`, `OrgResolver` |
...
| Test | Ginkgo `Describe/When/It` sentences; one `Test<Pkg>` bootstrap per package | `It("returns the period by id")` |
...
| Spec variables | two `var` blocks — inputs and fakes, then `result`/`err` (`002` §9) | `var ( svc *Service; result Period; err error )` |
...
**Error strings are fully lowercase — including acronyms.** Write `"invalid message id"` not `"invalid message ID"`, because error strings are often concatenated with other context (`fmt.Errorf("parsing token: %w", err)`) and mixed case looks wrong mid-sentence. Sentinel messages carry no package prefix: `errors.New("period not found")` (`.claude/rules/002-go-conventions.md` §4.1).
...
**Spec names:** lowercase behavior sentences in `It("…")` — `It("returns the period by id")`, not `"Valid Input"`. `t.Run()` subtests exist only in the rare plain-`testing` `_internal_test.go` file and follow the same casing.
...
- **[Types, Constants & Errors](./references/types-errors.md)** — Interface naming (by role; `-er` only for genuine agents), struct naming (nouns, MixedCaps), constants (MixedCaps, not ALL_CAPS), enums (type name prefix like `StatusReady`), sentinel errors (`ErrNotFound` variables), error types (`PathError` suffix), and error message conventions (lowercase, no punctuation).

- **[Test Naming](./references/testing.md)** — for the rare plain-`testing` `_internal_test.go` file only; specs are Ginkgo (`.claude/rules/006-testing.md`).
...
| `GetName()` getter | Go omits `Get` on a field accessor: `user.Name()`. Query methods on a store or service keep it — `GetUserByID`, `GetTemplateByID` are house style, never renamed. `Is`/`Has`/`Can` stay for boolean predicates |
...
Applying these fixes means renaming existing identifiers — → See `gopls` (optional tooling; the gate is `make lint && make test`) to do it safely: its rename updates every call site across the workspace and refuses a rename that would break interface satisfaction, which a grep/sed or manual Edit-based rename silently misses.

## Enforce with Linters

Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `.golangci.yml` (the pinned linter set; `make lint`) for configuration and usage.

## Cross-References

- → See `.claude/skills/golang-code-style/SKILL.md` for broader formatting and style decisions
- → See `.claude/rules/002-go-conventions.md` §3 (consumer-side interfaces) and §6 (pointer vs value) for interface naming depth and receiver design
- → See `.golangci.yml` (the pinned linter set; `make lint`) for automated enforcement (revive, predeclared, misspell, errname)
- → See `gopls` (optional tooling; the gate is `make lint && make test`) for safe rename when applying a naming fix
- → See `.claude/skills/golang-refactoring/SKILL.md` for how to apply a rename safely at scale (gopls Rename/Inline, blast-radius mapping, staged PR workflow) once you've decided what to rename identifiers to

...
Maps MUST NOT be accessed concurrently — → see `.claude/rules/002-go-conventions.md` §5 (concurrency & context, `pkg/appctrl`) for sync primitives.
...
For integer overflow as a security vulnerability, see `.claude/rules/004-security.md` section.
...
→ See `.claude/rules/002-go-conventions.md` §5 (concurrency & context, `pkg/appctrl`) for goroutine lifecycle and leak prevention.
...
→ See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for why init() should be avoided in favor of explicit constructors.

## Enforce with Linters

Many safety pitfalls are caught automatically by linters: `errcheck`, `forcetypeassert`, `nilerr`, `govet`, `staticcheck`. See `.golangci.yml` (the pinned linter set; `make lint`) for configuration and usage.
...
| Multiple `init()` with ordering assumptions | `init()` execution order across files is unspecified. → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) — use explicit constructors |
| Blocking forever on nil channel | Nil channels block on both send and receive. Always initialize before use |

## Cross-References

- → See `.claude/rules/002-go-conventions.md` §5 (concurrency & context, `pkg/appctrl`) for concurrent access patterns and sync primitives
- → See `.claude/skills/golang-error-handling/SKILL.md` for nil error interface trap
- → See `.claude/rules/004-security.md` for security-relevant safety issues (memory safety, integer overflow)

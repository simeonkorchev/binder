...
Upgrade to golangci-lint v2. **Migration**: Run `golangci-lint migrate` to convert v1 config to v2. See `.golangci.yml` (the pinned linter set; `make lint`) for the recommended configuration.
...
See `.github/workflows/quality-gate.yml` for ready-to-use GitHub Actions assets for both Claude Code and GitHub Copilot.

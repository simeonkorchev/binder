...
Use `%w` everywhere, the HTTP boundary included. What keeps callers from depending on internal error types is the domain's `serviceErrorMappings` table behind `handleErr` (`.claude/rules/002-go-conventions.md` §4.4): unmatched errors leave as opaque 500s, matched sentinels as their decided status. `%v` is never the mechanism — it silently breaks `errors.Is`.
...
// ✓ Later (Go 1.26+; go.mod is 1.25.6 — not yet) — same behavior, simpler syntax

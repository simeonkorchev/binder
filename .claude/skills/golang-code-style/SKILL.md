...
description: "Golang code style conventions — line length and breaking, variable declarations, control flow clarity, when comments help vs hurt. Use when writing or reviewing Go code, asking about style or clarity, or establishing project coding standards. Not for naming conventions (→ See `.claude/skills/golang-naming/SKILL.md`), linter configuration (→ See `.golangci.yml` (the pinned linter set; `make lint`)), or doc comments."
user-invocable: true
license: MIT
...
Style rules that require human judgment — linters handle formatting, this skill handles clarity. For naming see `.claude/skills/golang-naming/SKILL.md`; for design patterns see `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors); for struct/interface design see `.claude/rules/002-go-conventions.md` §3 (consumer-side interfaces) and §6 (pointer vs value).
...
`lll` enforces 120 columns in `make lint` (a `//nolint:lll` needs a reason). Signatures past 5 arguments / 3 results take an input struct from `model/` instead (`revive`, `.claude/rules/002-go-conventions.md` §2). Break at **semantic boundaries**, not arbitrary column counts. Function calls with 4+ arguments MUST use one argument per line — even when the prompt asks for single-line code:
...
- Functions SHOULD have **≤4 parameters**. Beyond that, use an options struct (see `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors)).
...
- **Unexport aggressively** — you can always export later; unexporting is a breaking change. → See `gopls` (optional tooling; the gate is `make lint && make test`) to unexport safely — its rename updates every call site atomically and refuses the change when lowercasing a method would break interface satisfaction, a breakage grep/sed silently ships.
...
The enforced set is `.golangci.yml` (57 linters, version pinned in `.golangci-version`); `make lint` runs it. Do not propose linter additions here — a linter change is its own PR.

## Cross-References

- → See `.claude/skills/golang-naming/SKILL.md` for identifier naming conventions
- → See `.claude/rules/002-go-conventions.md` §3 (consumer-side interfaces) and §6 (pointer vs value) for pointer vs value receivers, interface design
- → See `.claude/rules/000-principles.md` §7 (SOLID) and `002` §1.2 (constructors) for functional options, builders, constructors
- → See `.golangci.yml` (the pinned linter set; `make lint`) for automated formatting enforcement
- → See `.github/workflows/quality-gate.yml` for automated AI-driven code review in CI using these guidelines
- → See `.claude/skills/golang-refactoring/SKILL.md` for mechanically applying guard-clause conversion, function extraction, and options-struct migration safely across many call sites once a review surfaces violations at scale

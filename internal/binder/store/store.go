// Package store is the binder domain's SQL. Every method runs inside InTx, so a
...
// Store reads and writes binders and their slots.
type Store struct {
	db *sqlx.DB
}
...
// enough for everything in this package: the position rearrangements take their
// decisions from rows they have locked by writing to them, and the
// UNIQUE (binder_id, position) constraint is what rejects a racing writer.

...
// newTestHandler registers the domain's real operations on a real Huma API, so a
// spec exercises routing, validation, the Resolve hooks and serialisation rather
// than calling a handler function directly.
...
func do(svc api.Service, actor api.ActorFunc, method, target string, body any) *httptest.ResponseRecorder {
	return doWithHeaders(newTestHandler(svc, actor), method, target, body, nil)
}

// doWithHeaders runs one request through an arbitrary handler, which is how the
// middleware specs drive a real request past Middleware and into an endpoint.
func doWithHeaders(
	handler http.Handler, method, target string, body any, headers map[string]string,
...
// decodeBody reads a response body into target, failing the spec rather than the
// caller when it is not the JSON the endpoint promised.

...
- [x] T048 Commit the reviewed session into the binder — one call, one transaction — **both halves done.** `POST /binders/{binderId}/slots/batch` appends a whole reviewed sweep in one `InTx`: a bad card, a taken position or a failure anywhere in the commit rolls the entire batch back, so the binder never holds the prefix of a sweep the user has already put away (`internal/binder/store/batch_test.go` proves it, including that the batch rolls back with the caller's transaction). Bounded at `model.MaxSlotsPerBatch` = 450, enforced at the boundary by `maxItems` and again in the service; an empty commit is a 201 with `[]`. Two contract lies on the path to it were fixed at the same time: a scan now carries an `outcome` (`resolved` / `card_only` / `ambiguous` / `no_match`) so a client no longer has to read `resolution` for a question it cannot answer, and a printing the user picked is filed as the new `manual` rung rather than borrowing `exact`. Design and reasoning: `specs/001-binder-mvp/addendum-batch-commit.md`. **The client half** (2282f1d, 01231f4, c109f21, e90188d, 36c2beb, c2d18b2): `features/scan/api/toSlotCards.ts` shapes the sweep into the batch's cards — one entry per *physical* card, so the one decision a row carries is expanded back out to the `copies` it covered — and it is where the resolution rule lives: a printing the user picked is `manual`, a card the ladder settled keeps the rung it was settled on, a card kept with its set left open is `by_name`, and a card left out or never answered is not sent at all. A pure mapper with a completeness test, because a wrong resolution here is a slot that lies about how its set was found forever after, and the printing/resolution pair the `binder_slots` CHECK ties together is refused here rather than sent — one illegal row rolls the whole sweep back. `api/useCommitSweep.ts` sends it through `lib/apiRequest` (so it carries the bearer token and a 401 clears the session, unlike the scanner's two older calls) as **one** request, never a loop: a loop over the single-slot write is exactly the prefix-of-a-sweep the transaction exists to prevent. The cards are derived from the session's decisions on every render rather than copied into state, so **a failed commit loses nothing** — the captures, the answers and every decision are still there and pressing a binder again sends the same batch. A landed commit is the one thing that clears the sweep, so cards already in a binder cannot be filed twice. Which binder: the review sheet's third view shows `features/binder`'s own `BinderPicker` — the binders tab's list, extracted so both flows ask the question once, with the row's accessible name supplied by the caller — and creating a binder there picks it, so a collector with no binder does not have to abandon a reviewed sweep to make one. A filed sweep closes the sheet (a `Modal` is a native overlay, so the binder would open underneath it) and pushes `BinderPage`. Covered: `manual` vs the ladder's own rung vs `by_name`, one request and not a loop, a failed commit keeping every decision and retrying with the same body, an empty commit where every card was left out, and the user landing in the binder that was filled. **The two new views — the sheet's footer and the file view — have no screenshots in either theme**, for the same reason as T038; see T072
...
- [x] T080 **W14** [US1][US4][US6] Apple + Google sign-in (D1) via Expo AuthSession, exchanged at `POST /auth/sessions` for the session JWT the backend issues — `apps/mobile/src/features/auth/`. `requestProviderIdentity` is the one place that knows Apple from Google (mirroring `internal/user/identity`), `exchangeIdentityToken` posts the provider token, `useSignIn` runs the three steps; the nonce is minted once per attempt and the value the provider is given is the value the backend is told, which is what `pkg/oidc.checkNonce` compares. Neither provider is asked for a name or an email — `oidc.Identity` carries the subject and nothing else. Four outcomes, because a cancelled sheet, a provider this build cannot offer and a refused token are not one failure; a refusal is one sentence with no detail, and the token, the nonce and the authorization code never reach a message or a log. **One deviation from D1, stated plainly: Google goes through Expo AuthSession (code flow + PKCE), Apple goes through the native `expo-apple-authentication` sheet.** Apple's web flow returns the identity token by `form_post` to an **https** redirect, which an app with a custom URI scheme cannot receive and this backend has no endpoint for — AuthSession for Apple would ship a button that cannot work. **No live Apple or Google sign-in was performed** (impossible in this container; `appleid.apple.com` is 403 at the proxy), so two provider behaviours are assumed from their documented contracts and are the first things to check on a real device: that Apple copies the request nonce into the token's `nonce` claim unchanged, and that Google accepts a redirect built from the application id
- [x] T081 **W14** Session stored in **SecureStore**, not AsyncStorage: it is a bearer credential — `src/lib/sessionStore.ts` with the pure shape in `src/lib/storedSession.ts`. A store rather than a context because the request helper is a plain module that has to read the token and must not become a hook; React subscribes through `useSyncExternalStore`. The launch read is the **synchronous** `SecureStore.getItem`, so the answer is there in the app's first effect and nothing renders against an undecided session; writes stay async. Only the token, the expiry and the user id are stored — the published contact details stay out of the keychain. A stored value that is not a session (older build, half-written) ends in "sign in again" rather than in `Bearer undefined`; a token whose 24h has run out is dropped rather than sent, because there is no refresh token. Cleared on sign-out (a button in the binders header) and on any 401
- [x] T082 **W14** `lib/apiRequest.ts` attaches the bearer token, and a 401 sends the user back to sign-in rather than surfacing as a generic failure. One place builds the header, so no hook takes a token and no component holds one; signed out there is no `Authorization` header at all rather than an empty one. A 401 is the only status the helper acts on: it drops the session before throwing, which publishes `signed-out` and swaps the navigator's stack, so no screen has to navigate. The module doc claimed the opposite and was rewritten with it. The one 401 a signed-out visitor can provoke — the seller contact reveal, which needs an actor by design — now says "sign in to see how to reach this seller" instead of "could not be loaded"
- [x] T083 **W14** A signed-out user sees the sign-in screen; a signed-in one lands on their binders. `GET /listings` stays reachable signed-out (browse is public by design): the sign-in screen offers "Browse what is for sale", which pushes the same market screen, and `App.test.tsx` and `MarketScreen.test.tsx` both pin that the feed is read with **no** `Authorization` header. Signed in, the tabs open on Binders — the scanner has nowhere to put a card until a binder exists. **Screenshots not taken** for the sign-in screen, in either theme, for the same reason as T038 — see T072; adding native modules also means the dev client must be rebuilt before it can run at all
...
- [ ] T072 UI screenshots of every new screen in BOTH themes (`CLAUDE.md` requires this before any UI change is done) — **still blocked in this container**: no emulator, no `adb`, no Android SDK, and the scanner is a dev build, so neither Expo Go nor a web preview can render it. Blocks ticking T038, T040, T041, T046 and T047, covers T048's two new views (the review sheet's file footer and the binder picker it opens), and the sign-in screen from T083 is outstanding for the same reason
- [x] T073 Clean-code checklist — `.claude/rules/007-clean-code-checklist.md` walked over every touched file, both layers. **Go** (a52a706, 41a219b, 952ee03): one LIKE escaper in `pkg/sqllike` for the two stores that each had their own; one error-table mechanism in `pkg/humaerr` for the four domains that each re-declared it; `slices.Contains` for two hand-rolled membership loops; `NewStore` in the card store, so the four stores agree. Mapper totality is guarded by `exhaustruct`'s include list plus the `*_internal_test.go` completeness tests, and every `//nolint` names its linter and a reason. **Frontend** (958bc32, 0e47952, 0426620): two comments claiming `GET /listings` was the only endpoint needing no actor, which the card domain's two have never needed either; `nav.binderPage` and `binder.add.retry`, in both locales and read nowhere; the card search moved to a `features/card/` of its own, which is what its own comment asked for once W9 was finished, so the binder no longer reaches into the scanner's `api/` and three aliases of `ScanCard` became two; `ScanSession.capturedCount` and `pendingCount`, which only their own suite read. **Four findings are logged rather than fixed**, all of them behaviour-changing (`.ai/findings/open/`): the scan feature's two calls bypassing `lib/apiRequest`; `apps/mobile` having no React Query against `003-frontend.md` §1b, with the same fetch state machine hand-rolled in four features (**high** — the largest piece of cleanup left in the app); the same "could not be loaded" block in four sizes across four screens, which cannot be unified without the screenshots T072 is blocked on; and the pocket sheet reusing the add sheet's note for a card that *was* scanned. `make check` green, run to completion: 0 lint issues, 26 Ginkgo suites under `-race`, composite coverage 87.2%, mobile lint + typecheck + 466 tests across 47 suites + knip, `check-contract` regenerating with no diff, `check-ci-parity` green, working tree clean afterwards

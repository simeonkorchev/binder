...
- [ ] T048 Commit the reviewed session into the binder — one call, one transaction — **server half done, client half outstanding.** `POST /binders/{binderId}/slots/batch` appends a whole reviewed sweep in one `InTx`: a bad card, a taken position or a failure anywhere in the commit rolls the entire batch back, so the binder never holds the prefix of a sweep the user has already put away (`internal/binder/store/batch_test.go` proves it, including that the batch rolls back with the caller's transaction). Bounded at `model.MaxSlotsPerBatch` = 450, enforced at the boundary by `maxItems` and again in the service; an empty commit is a 201 with `[]`. Two contract lies on the path to it were fixed at the same time: a scan now carries an `outcome` (`resolved` / `card_only` / `ambiguous` / `no_match`) so a client no longer has to read `resolution` for a question it cannot answer, and a printing the user picked is filed as the new `manual` rung rather than borrowing `exact`. Design and reasoning: `specs/001-binder-mvp/addendum-batch-commit.md`. **Still to do on the app side** (deliberately not touched here, another change was mid-wave in `apps/mobile/`): send the sweep to the batch endpoint, and file a user-picked printing as `manual`. The third item — `lib/flaggedScans.ts` classifying on `outcome` instead of sniffing the answer's shape — landed with the review sheet and is done; T073 checked it against the code **UNTICKED 2026-09-18: the client half genuinely does not exist.** Nothing in `apps/mobile` calls the batch endpoint — the sweep runs, the review sheet collects decisions, and the cards never reach a binder. The core product loop is open, and a ticked box hid it. The endpoint is published in `packages/types` and tested; what is missing is the call, the `manual` resolution a user-picked printing must carry, and the navigation from a committed sweep to the binder it filled

...
export default ({ config }: ConfigContext): ExpoConfig => ({
  ...
  version: '0.0.0',
  // Both written by hand, not by `eas init`: this is a dynamic config (a
  // function), and the CLI only patches a static app.json — it prints the id
  // and asks you to add it yourself.
  //
  // `owner` is the guard rather than decoration. eas-cli keeps one login at a
  // time in ~/.expo/, shared with every other Expo project on the machine, so
  // without this a build run under the wrong account creates a stray project
  // there instead of failing. Naming the owner makes that an error.
  owner: 'simonadkorchevas-team',
